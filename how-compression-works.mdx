...
Summaries are cached in two places on your machine: an in-memory cache in the running gateway for fast reuse within a session, and the local CMX store for reuse across restarts. Both caches are scoped to the upstream endpoint, model, credential scope, and source content. Cached islands are applied for free on later requests and do not count against the per-request spend limit.
