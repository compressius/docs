...
<Steps>
  <Step title="Install CMX">
    ...
    The curl installer lists the coding clients and providers that are ready to connect, then asks `Configure all listed providers now? [y/N]`. Enter `y` to route them through CMX now. This also turns on a check, run at most once every 24 hours, that connects agents you add later. Press Enter or type `n` to leave your client settings unchanged and keep that check off. You can connect agents later with the steps below or with `cmx setup --connect-ready`. See [Connect ready agents during install](/installation#connect-ready-agents-during-install) for details.
  </Step>
