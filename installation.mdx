...
<Tabs>
  ...
  <Tab title="Windows">
    ...
    - [Windows x64 installer](https://compressi.us/downloads/cmx-setup-windows-amd64.exe) — Intel and AMD PCs
    - [Windows ARM64 installer](https://compressi.us/downloads/cmx-setup-windows-arm64.exe) — Windows on ARM
  </Tab>
</Tabs>
...
Nightly builds ship the latest fixes and features ahead of a stable release. Each nightly build has an immutable version string; stable is still the recommended channel for everyday use.
...
<CodeGroup>
  ...
  ```sh curl nightly (Linux / macOS)
  curl -sSfL https://compressi.us/install-nightly.sh | sh
  ```
</CodeGroup>

For standalone nightly files, select a prerelease from the [releases page](https://github.com/compressius/cmx/releases) and verify the download against the matching `SHA256SUMS` file.

<Warning>
  Nightly builds are for testing and may contain breaking changes. Use the stable channel for production coding sessions.
</Warning>

## Uninstall

Disconnect your coding agents before removing CMX so they resume talking directly to your provider.
...
<Steps>
  ...
  <Step title="Remove the package">
    Run the command that matches how you installed CMX:
    ...
    **Standalone install:** Locate the binary with `command -v cmx` (Linux / macOS) or `Get-Command cmx` (PowerShell), then delete that file from its installation folder. If you configured an auto-start service manually, disable it as well.
  </Step>
</Steps>
